...
AgvCmdQueue::~AgvCmdQueue()
{
    ...
    while(!threadAlreadQuit)
    {
        ...
        std::this_thread::sleep_for(dura);//阻塞当前线程
    }
}

void AgvCmdQueue::cmdThread(void *param)//void *是一开始不知道指向什么类型，后期强制类型转换
{
...
void AgvCmdQueue::cmdProcess()
{
    ...
    while(!quit)
    {
        ...
        //第一次发送任务
        if(ordersSendIndex == 0){//当前发送指令的序列号
            sendOrder();
        }else{
...
{
    mtx.lock();
    if(ordersSendIndex>=orders.length()){//执行玩所有队列中的指令
        orders.clear();
        ordersSendIndex = 0;
    ...
    //根据orders封装一个发送的命令
    QByteArray content; // content可以是由agvorder结构组成的
